 ...
 */
public class MemoryStoreFlightHolder implements AutoCloseable {
   ...
   */
  public MemoryStoreFlightHolder(BufferAllocator allocator, FlightDescriptor descriptor, Schema schema,
   ...
   */
  public Stream getStream(MemoryStoreTicket ticket) {
  ...
  public FlightInfo getFlightInfo(final Location l) {
    ...
    for (Stream s : streams) {
      endpoints.add(
          new FlightEndpoint(
              new MemoryStoreTicket(descriptor.getPath(), i, s.getUuid())
                .toTicket(),
                l));
      i++;
    }
